...
const PLCRVoting = artifacts.require('PLCRDelegateVoting.sol');
...
const utils = {
  ...
  commitVoteOnBehalf: async (owner, pollID, voteOption, tokensArg, salt, voter, voting) => {
    const hash = utils.getVoteSaltHash(voteOption, salt);
    await utils.as(owner, voting.requestDelegateVotingRightsTo, voter, tokensArg);
    const prevPollID = await voting.getInsertPointForNumTokens.call(owner, tokensArg, pollID);
    await utils.as(voter, voting.commitVoteOnBehalf, owner, pollID, hash, tokensArg, prevPollID);
  },
